import 'package:flutter/material.dart';
import 'package:supabase_flutter/supabase_flutter.dart';

class TranslationService extends ChangeNotifier {
  static TranslationService _instance = TranslationService._internal();
  factory TranslationService() => _instance;
  TranslationService._internal();

  @visibleForTesting
  static void setMockInstance(TranslationService mock) {
    _instance = mock;
  }

  String _currentLanguage = 'Français';
  String get currentLanguage => _currentLanguage;

  String _currentCurrency = 'FCFA';
  String get currentCurrency => _currentCurrency;

  void setLanguage(String language) {
    _currentLanguage = language;
    _syncWithDatabase();
    notifyListeners();
  }

  void setCurrency(String currency) {
    _currentCurrency = currency;
    _syncWithDatabase();
    notifyListeners();
  }

  Future<void> loadUserPreferences() async {
    try {
      final user = Supabase.instance.client.auth.currentUser;
      if (user != null) {
        final response = await Supabase.instance.client
            .schema('cartel')
            .from('profiles')
            .select('language_preference, currency_preference')
            .eq('id', user.id)
            .maybeSingle();

        if (response != null) {
          if (response['language_preference'] != null) {
            _currentLanguage = response['language_preference'];
          }
          if (response['currency_preference'] != null) {
            _currentCurrency = response['currency_preference'];
          }
          notifyListeners();
        }
      }
    } catch (e) {
      debugPrint('Error loading preferences: $e');
    }
  }

  Future<void> _syncWithDatabase() async {
    try {
      final user = Supabase.instance.client.auth.currentUser;
      if (user != null) {
        await Supabase.instance.client
            .schema('cartel')
            .from('profiles')
            .update({
          'language_preference': _currentLanguage,
          'currency_preference': _currentCurrency,
        }).eq('id', user.id);
      }
    } catch (e) {
      debugPrint('Error syncing preferences: $e');
    }
  }

  String formatPrice(double priceInFCFA) {
    if (_currentCurrency == 'USD') {
      double usdPrice = priceInFCFA / 600;
      if (usdPrice >= 1000000) {
        return '${(usdPrice / 1000000).toStringAsFixed(1)}M USD';
      }
      return '\$${usdPrice.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
    } else {
      if (priceInFCFA >= 1000000) {
        return '${(priceInFCFA / 1000000).toStringAsFixed(1)}M FCFA';
      }
      return '${priceInFCFA.toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]}.')} FCFA';
    }
  }

  static const Map<String, Map<String, String>> _translations = {
    'English': {
      'welcome_back': 'Welcome Back,',
      'signup_title': 'Join the Elite',
      'signup_subtitle': 'Experience the future of luxury car sourcing',
      'full_name': 'Full Name',
      'email': 'Email',
      'password': 'Password',
      'forgot_password': 'Forgot Password?',
      'forgot_password_subtitle': 'Enter your email address to receive account reset instructions.',
      'recovery_email': 'Recovery Email',
      'send_link': 'Send Link',
      'back_to_login': 'Back to Login',
      'need_help': 'Need Help?',
      'contact_support': 'Contact Support',
      'reset_link_sent': 'Reset link sent to your email',
      'reset_password_title': 'Reset Password',
      'reset_password_subtitle': 'Please create a new secure password for your CarTel account.',
      'new_password': 'New Password',
      'confirm_password': 'Confirm Password',
      'reset_access_btn': 'Reset My Access',
      'min_8_chars': 'Minimum 8 characters',
      'one_special_char': 'One special character',
      'one_number': 'One number',
      'password_reset_success': 'Password reset successfully',
      'or_continue_with': 'Or Continue With',
      'google': 'Google',
      'apple': 'Apple',
      'phone_number': 'Phone Number',
      'country_residence': 'Country of Residence',
      'i_accept_terms': 'I accept the terms and conditions',
      'log_in_btn': 'Log In',
      'signup_btn': 'Sign Up',
      'new_to_cartel': 'New to CarTel?',
      'already_have_account': 'Already have an account?',
      'take_photo': 'Take a photo',
      'choose_gallery': 'Choose from gallery',
      'select_country_error': 'Please select your country of residence',
      'enter_phone_error': 'Please enter your phone number',
      'accept_terms_error': 'Please accept the terms of use',
      'error_occurred': 'An error occurred',
      'select_make_error': 'Please select a brand',
      'model_required_error': 'Please enter a model',
      'select_country': 'Select a country',
      'search_country': 'Search for a country...',
      'votre_experience': 'Your Experience',
      'partagez_avis': 'Share Your Review',
      'avis_subtitle': 'Your testimony helps the CarTel community and allows us to improve our services.',
      'vehicule_livre': 'Vehicle Delivered',
      'notez_experience': 'Rate Your Experience',
      'commentaire_label': 'Comment',
      'commentaire_hint': 'Tell us how your search and delivery went...',
      'photo_vehicule': 'Vehicle Photo',
      'ajouter_photo': 'Add a photo',
      'photo_specs': 'JPG, PNG up to 10 MB',
      'publier_avis': 'Publish My Review',
      'leave_review': 'Leave A Review',
      'give_rating_error': 'Please give a rating',
      'thank_you_review': 'Thank you for your review!',
      'error_prefix': 'Error',
      'profile_updated': 'Profile updated successfully',
      'error': 'Error',
      'nouveau_ici': 'New Here?',
      'comment_ca_marche': 'How it works?',
      'trending_now': 'Trending Now',
      'view_all': 'View All',
      'experiences_clients': 'Customer Experiences',
      'home': 'Home',
      'demandes': 'Requests',
      'alerts': 'Alerts',
      'profile': 'Profile',
      'select_language': 'Select Language',
      'language_subtitle': 'Personalize your luxury experience with your preferred language.',
      'continue': 'Continue',
      'select_currency': 'Select Currency',
      'currency_subtitle': 'Choose your preferred currency for all car sourcing requests.',
      'notifications': 'Alerts',
      'mark_all_read': 'Mark all read',
      'mes_demandes': 'My Requests',
      'actives': 'Actives',
      'toutes': 'All',
      'initialisee': 'Initialized',
      'initialisée': 'Initialized',
      'en_cours': 'In Progress',
      'en cours': 'In Progress',
      'in_progress': 'In Progress',
      'in progress': 'In Progress',
      'terminee': 'Completed',
      'terminée': 'Completed',
      'budget_max': 'Max Budget',
      'agent': 'Agent',
      'details': 'Details',
      'details_demande': 'Request Details',
      'trouve': 'Found',
      'budget_prevu': 'Planned Budget',
      'periode': 'Period (Years)',
      'matches_trouves': 'Matches Found',
      'voir_tout': 'See all',
      'accepter': 'Accept',
      'agent_responsable': 'Responsible Agent',
      'prix_final': 'Final Price',
      'couleur': 'Color',
      'etat': 'Condition',
      'compris_y_vais': 'Understood, I\'m Going',
      'creez_demande_title': '1. Create your request',
      'creez_demande_desc': 'Submit your criteria (brand, model, year, budget) via our Standard, Bundle or Spot forms.',
      'selectionnez_vehicule_title': '2. Select your vehicle',
      'selectionnez_vehicule_desc': 'Once the search is finished, choose the rare gem from the list of vehicles found by our agents.',
      'paiement_securise_title': '3. Secure payment',
      'paiement_securise_desc': 'Make the payment for the purchase of the vehicle safely via our certified payment channels.',
      'remise_cles_title': '4. Handing over the keys',
      'remise_cles_desc': 'Receive your keys in person within 3 days after the finalization of the purchase of the vehicle.',
      'suivi_livraison_title': '5. Delivery tracking',
      'suivi_livraison_desc': 'As soon as the vehicle is shipped, enjoy real-time tracking of the cargo to the port of arrival.',
      'user_guide': 'User Guide',
      'cartel_private_sourcing': 'Cartel Private\nSourcing',
      'demande_cartel_aujourdhui': 'CarTel Request • Today',
      'demande_cartel_10_oct': 'CarTel Request • 10 Oct',
      'demande_cartel_08_oct': 'CarTel Request • 08 Oct',
      'identification_vehicules': 'Vehicle identification',
      'attente_assignation_agent': 'Waiting for agent assignment',
      'livre': 'Delivered',
      'vehicule_trouve': 'Vehicle Found',
      'statut': 'Status',
      'resultat': 'Result',
      'paiement': 'Payment',
      'confirme': 'Confirmed',
      'top_match': 'Top Match',
      'hors_budget': 'Over Budget',
      'chat_agent': 'Chatting with agent...',
      'match_accepte': 'Offer accepted',
      'match_refuse': 'Offer declined',
      'accepter_offre': 'Accept this offer?',
      'confirm_accept_match_desc': 'Do you want to accept this offer? This action is irreversible.',
      'refuser_offre': 'Decline this offer?',
      'confirm_decline_match_desc': 'Do you want to decline this offer?',
      'match_accepte_success': 'Offer accepted successfully',
      'match_refuse_success': 'Offer declined successfully',
      'traitement': 'Processing...',
      'notif_match_title': 'New Vehicle Match',
      'notif_match_desc': 'Agent Marcus found a 2023 Porsche Cayenne Coupe S matching your criteria.',
      'notif_assign_title': 'Agent Assigned',
      'notif_assign_desc': 'Specialist Marcus K. has been assigned to your search #CT-8921.',
      'notif_pay_title': 'Payment Confirmed',
      'notif_pay_desc': 'Service fee of 30,000 FCFA received for request #CT-8921.',
      'starting_from': 'Starting From',
      'just_now': 'Just now',
      '2_hours_ago': '2 hours ago',
      'yesterday': 'Yesterday',
      'hero_title': 'Let us find your ideal vehicle.',
      'hero_subtitle': 'Personalized search, secure purchase and delivery.',
      'modele_2024': '2024 Model',
      'testimonial_text': '"CarTel sourced my G-Wagon directly from Dubai. The transit updates were precise and delivery at the port was seamless."',
      'preferences': 'Preferences',
      'support_legal': 'Support & Legal',
      'premium_member': 'Premium Member',
      'currency': 'Currency',
      'help_center': 'Help Center',
      'privacy_policy': 'Privacy Policy',
      'contact': 'Contact',
      'help_center_title': 'At your service',
      'help_center_subtitle': 'Our team of experts is here to accompany you in your automotive search.',
      'write_us': 'Write us',
      'follow_us': 'Follow us',
      'customer_service_hours': 'Customer service available 7/7\nFrom 08:00 to 20:00 (GMT+1)',
      'log_out': 'Log Out',
      'consulter_rapport': 'Consult final report',
      'country_of_residence': 'Country of residence',
      'pricing_from': 'Pricing From',
      'model_label': 'Model',
      'start_sourcing': 'Start Sourcing',
      'kilométrage': 'Mileage',
      'énergie': 'Energy',
      'fuel_petrol': 'Petrol',
      'fuel_diesel': 'Diesel',
      'fuel_electric': 'Electric',
      'fuel_hybrid': 'Hybrid',
      'trans_auto': 'Auto',
      'trans_manual': 'Manual',
      'payment_success': 'Payment Successful',
      'receipt_email_sent': 'A receipt has been sent to your email address.',
      'payment_failed': 'Payment Declined',
      'try_again': 'Try Again',
      'change_method': 'Change Payment Method',
      'track_my_request': 'Track My Request',
      'back_to_home': 'Back To Home',
      'transaction_id': 'Transaction ID',
      'service_fee': 'Service Fee',
      'amount': 'Amount',
      'error_code': 'Error Code',
      'active_search': 'Active Search',
      'transaction_failed': 'Transaction Failed',
      'payment_activated_desc': 'Your search request has been activated. A dedicated agent will begin sourcing your Porsche Urus immediately.',
      'payment_failed_desc': "We couldn't process your payment. Please verify your balance or try a different method.",
      'create_request': 'Create Request',
      'vehicle_identity': 'Vehicle Identity',
      'brand': 'Brand',
      'investment_range': 'Investment Range',
      'minimum': 'Minimum',
      'maximum': 'Maximum',
      'production_year': 'Production Year',
      'condition': 'Condition',
      'new': 'New',
      'used': 'Used',
      'aesthetics': 'Aesthetics',
      'exterior_shade': 'Exterior Shade',
      'interior_shade': 'Interior Shade',
      'special_requirements': 'Special Requirements',
      'describe_requirements': 'Describe any specific options or preferences...',
      'submit_request': 'Submit Request',
      'optional': 'Optional',
      'all': 'All',
      'suv': 'SUV',
      'sports': 'Sports',
      'sedan': 'Sedan',
      'coupe': 'Coupe',
      'convertible': 'Convertible',
      'hatchback': 'Hatchback',
      'pickup': 'Pickup',
      'wagon': 'Wagon',
      'van': 'Van',
      'awd': 'AWD',
      'fwd': 'FWD',
      'rwd': 'RWD',
      '4wd': '4WD',
      '4x4': '4x4',
      'mileage_range': 'Mileage Range',
      'kilometers': 'km',
      'accept_terms_prefix': 'I accept the',
      'terms_link': 'Terms of Use',
      'and_label': 'and the',
      'privacy_link': 'Privacy Policy',
      'create_account_btn': 'Create my account',
      'login_title': 'Welcome Back',
      'login_subtitle': 'Log in to continue your search',
      'no_requests_found': 'No requests found',
      'no_trending_cars': 'No trending cars available',
      'expertise_agent': 'Expertise of the Agent',
      'default_expertise_note': "This vehicle is in exceptional condition, first hand, with a up-to-date Mercedes maintenance book. The AMG Line pack and panoramic sunroof make it a rare configuration on the market.",
      'verdict_agent': 'Agent\'s Verdict',
      'specialiste_luxe': 'Luxury & Performance Specialist',
      'fiche_technique': 'Technical Specifications',
      'marque': 'Make',
      'modele': 'Model',
      'annee': 'Year',
      'kilometrage': 'Mileage',
      'moteur': 'Engine',
      'boite': 'Transmission',
      'peinture_ext': 'Exterior',
      'cuir_int': 'Interior',
      'prix_exportation': 'Export Price',
      'price_label': 'Price',
      'hot_deals': 'Hot Deals',
      'affordable': 'Affordable',
      'best_price': 'Best Price',
      'interested': 'Interested',
      'delete_request_title': 'Delete Request?',
      'delete_request_msg': 'This action is irreversible. Do you want to continue?',
      'cancel': 'Cancel',
      'delete': 'Delete',
      'request_deleted': 'Request deleted successfully',
      'request_delete_error': 'Error: Could not delete request. RLS or incorrect ID.',
      'step': 'Step',
      'of': 'Of',
      'confirmation': 'Confirmation',
      'search_summary': 'Search Summary',
      'search_request': 'Search Request',
      'currency_label': 'Currency',
      'payment_method': 'Payment Method',
      'secure_transaction_msg': 'Secure and encrypted end-to-end transactions.',
      'pay_amount': 'Pay',
      'payment_failed_prefix': 'Payment failed',
      'payment_cancelled_prefix': 'Payment cancelled or failed',
      'unexpected_error': 'An unexpected error occurred',
      'onboarding': 'Onboarding',
      'identity': 'Identity',
      'condition_label': 'Condition',
      'investment': 'Investment',
      'specs': 'Specs',
      'custom': 'Custom',
      'e_g_urus': 'e.g. Urus',
      'no_deals_available': 'No deals available',
      'model_suffix': 'Model',
      'searching': 'Searching',
      'cancel_request': 'Cancel Request',
      'cartel_search': 'Cartel Search',
      'file_status': 'File Status',
      'no_matches_found': 'No matches found',
      'paiement_valide': 'Payment Validated',
      'agent_assigne': 'Agent Assigned',
      'attente_assignation': 'Waiting for assignment...',
      'recherche_active': 'Active Search',
      'identification_en_cours': 'Vehicle identification in progress...',
      'en_attente': 'Waiting...',
      'rapport_final': 'Final Report',
      'attente_resultats': 'Waiting for results',
      'expert_luxe_actif': 'Luxury Expert • Active',
      'note_logistique': 'Logistics Note',
      'shipping_fee_prefix': 'International shipping fees from',
      'no_notifications': 'No notifications available',
      'clear_all': 'Clear All',
      'clear_all_notifications_title': 'Clear all notifications?',
      'clear_all_notifications_desc': 'This action cannot be undone.',
      'swipe_to_decide': 'Swipe Right to Accept • Left to Decline',
      'unassigned': 'Unassigned',
      'match': 'Match',
      'found': 'Found',
      'assignment': 'Assignment',
      'agent_assigned': 'Assignment',
      'payment': 'Payment',
      'payment_confirmed': 'Payment',
      'current_selection': 'Current Selection',
      'customs_fees': 'Customs Fees',
      'customs_fees_msg': 'Customs clearance fees are not included in the displayed prices.',
      'online': 'Online',
      'luxury_expert': 'Luxury Expert',
      'typing': 'is typing...',
      'write_message': 'Write a message...',
      'today': 'Today',
      'chat_with_agent': 'Chat',
      'body_style': 'Body Style',
      'drive_train': 'Drive Train',
      'features': 'Features',
      'equipements_inclus': 'Included Equipment',
      'awaiting_payment': 'Awaiting Payment',
      'payment_pending_short': 'Pending',
      'payment_declined_short': 'Declined',
      'match_accepted_step': 'Match accepted · Pending payment',
      'match_paid_step': 'Final payment confirmed',
      'in_transit': 'In Transit',
      'in_transit_step': 'Vehicle in transit',
      'delivered_step': 'Vehicle delivered · Awaiting confirmation',
      'support_center': 'CarTel Support',
      'support_chat_empty': 'Send a message to start chatting with our support team.',
      'privacy_last_updated': 'LAST UPDATED: OCT 2023',
      'privacy_title': 'Privacy\nPolicy',
      'privacy_subtitle': 'At CarTel, we place the utmost importance on protecting your personal data. This policy details how we collect and process your information.',
      'privacy_data_collection': 'Data Collection',
      'privacy_data_collection_label': 'User-Provided Information',
      'privacy_data_collection_body': 'We collect your name, email, phone number, and details regarding your vehicle searches when you create requests.',
      'privacy_data_usage': 'Data Usage',
      'privacy_data_usage_label': 'Purpose of Processing',
      'privacy_data_usage_body': 'Your data is used exclusively to facilitate your vehicle search by our agents and to track your transactions.',
      'privacy_security': 'Security & Sharing',
      'privacy_security_body': 'CarTel never sells your data to third parties. We use advanced encryption protocols to ensure the integrity of your information.',
      'privacy_questions': 'Any questions?',
      'privacy_contact_subtitle': 'Contact our data protection officer at support@cartel.app',
      'privacy_contact_btn': 'Contact Support',
      'privacy_copyright': '© 2023 CarTel Automobile. All rights reserved.',
    },
    'Français': {
      'welcome_back': 'Bienvenue,',
      'signup_title': 'Rejoignez l\'Élite',
      'signup_subtitle': 'Vivez le futur du sourcing de voitures de luxe',
      'full_name': 'Nom Complet',
      'email': 'E-mail',
      'password': 'Mot de Passe',
      'forgot_password': 'Mot de passe oublié ?',
      'forgot_password_subtitle': 'Entrez votre adresse e-mail pour recevoir les instructions de réinitialisation de votre compte.',
      'recovery_email': 'E-mail de récupération',
      'send_link': 'Envoyer le lien',
      'back_to_login': 'Retour à la connexion',
      'need_help': 'Besoin d\'aide ?',
      'contact_support': 'Contactez le Support',
      'reset_link_sent': 'Lien de réinitialisation envoyé sur votre e-mail',
      'reset_password_title': 'Réinitialisation',
      'reset_password_subtitle': 'Veuillez créer un nouveau mot de passe sécurisé pour votre compte CarTel.',
      'new_password': 'Nouveau mot de passe',
      'confirm_password': 'Confirmer le mot de passe',
      'reset_access_btn': 'Réinitialiser mon accès',
      'min_8_chars': 'Minimum 8 caractères',
      'one_special_char': 'Un caractère spécial',
      'one_number': 'Un chiffre',
      'password_reset_success': 'Mot de passe réinitialisé avec succès',
      'or_continue_with': 'Ou Continuer Avec',
      'google': 'Google',
      'apple': 'Apple',
      'phone_number': 'Numéro de Téléphone',
      'country_residence': 'Pays de Résidence',
      'i_accept_terms': 'J\'accepte les conditions d\'utilisation',
      'log_in_btn': 'Se Connecter',
      'signup_btn': 'S\'Inscrire',
      'new_to_cartel': 'Nouveau sur CarTel ?',
      'already_have_account': 'Vous avez déjà un compte ?',
      'take_photo': 'Prendre une photo',
      'choose_gallery': 'Choisir dans la galerie',
      'select_country_error': 'Veuillez sélectionner votre pays de résidence',
      'enter_phone_error': 'Veuillez entrer votre numéro de téléphone',
      'accept_terms_error': 'Veuillez accepter les conditions d\'utilisation',
      'error_occurred': 'Une erreur est survenue',
      'select_make_error': 'Veuillez sélectionner une marque',
      'model_required_error': 'Veuillez entrer un modèle',
      'select_country': 'Sélectionnez un pays',
      'search_country': 'Rechercher un pays...',
      'votre_experience': 'Votre Expérience',
      'partagez_avis': 'Partagez Votre Avis',
      'avis_subtitle': 'Votre témoignage aide la communauté CarTel et nous permet d\'améliorer nos services.',
      'vehicule_livre': 'Véhicule Livré',
      'notez_experience': 'Notez Votre Expérience',
      'commentaire_label': 'Commentaire',
      'commentaire_hint': 'Racontez-nous comment s\'est déroulée votre recherche et la livraison...',
      'photo_vehicule': 'Photo du Véhicule',
      'ajouter_photo': 'Ajouter une photo',
      'photo_specs': 'JPG, PNG jusqu\'à 10 Mo',
      'publier_avis': 'Publier Mon Avis',
      'leave_review': 'Laisser Un Avis',
      'give_rating_error': 'Veuillez donner une note',
      'thank_you_review': 'Merci pour votre avis !',
      'error_prefix': 'Erreur',
      'profile_updated': 'Profil mis à jour avec succès',
      'error': 'Erreur',
      'nouveau_ici': 'Nouveau Ici ?',
      'comment_ca_marche': 'Comment ça marche ?',
      'trending_now': 'Tendances Actuelles',
      'view_all': 'Voir tout',
      'experiences_clients': 'Expériences Clients',
      'home': 'Accueil',
      'demandes': 'Demandes',
      'alerts': 'Alertes',
      'profile': 'Profil',
      'select_language': 'Choisir la langue',
      'language_subtitle': 'Personnalisez votre expérience de luxe avec votre langue préférée.',
      'continue': 'Continuer',
      'select_currency': 'Choisir la devise',
      'currency_subtitle': 'Choisissez votre devise préférée pour toutes vos demandes.',
      'notifications': 'Alertes',
      'mark_all_read': 'Tout marquer comme lu',
      'mes_demandes': 'Mes Demandes',
      'actives': 'Actives',
      'toutes': 'Toutes',
      'initialisee': 'Initialisée',
      'initialisée': 'Initialisée',
      'en_cours': 'En cours',
      'en cours': 'En cours',
      'in_progress': 'En cours',
      'in progress': 'En cours',
      'terminee': 'Terminée',
      'terminée': 'Terminée',
      'budget_max': 'Budget Max',
      'agent': 'Agent',
      'details': 'Détails',
      'details_demande': 'Détails de Demande',
      'trouve': 'Trouvé',
      'budget_prevu': 'Budget Prévu',
      'periode': 'Période (Ans)',
      'matches_trouves': 'Matches Trouvés',
      'voir_tout': 'Voir tout',
      'accepter': 'Accepter',
      'agent_responsable': 'Agent Responsable',
      'prix_final': 'Prix Final',
      'couleur': 'Couleur',
      'etat': 'État',
      'compris_y_vais': 'Compris, J\'y Vais',
      'creez_demande_title': '1. Créez votre demande',
      'creez_demande_desc': 'Soumettez vos critères (marque, modèle, année, budget) via nos formulaires Standard, Bundle ou Spot.',
      'selectionnez_vehicule_title': '2. Sélectionnez votre véhicule',
      'selectionnez_vehicule_desc': 'Une fois la recherche terminée, choisissez la perle rare parmi la liste des véhicules trouvés par nos agents.',
      'paiement_securise_title': '3. Paiement sécurisé',
      'paiement_securise_desc': 'Effectuez le règlement pour l\'achat du véhicule en toute sécurité via nos canaux de paiement certifiés.',
      'remise_cles_title': '4. Remise des clés',
      'remise_cles_desc': 'Recevez vos clés en main propre sous 3 jours après la finalisation de l\'achat du véhicule.',
      'suivi_livraison_title': '5. Suivi de livraison',
      'suivi_livraison_desc': 'Dès que le véhicule est expédié, profitez d\'un suivi en temps réel de la cargaison jusqu\'au port d\'arrivée.',
      'user_guide': 'Guide de l\'Utilisateur',
      'cartel_private_sourcing': 'Le Sourcing Privé\nCartel',
      'demande_cartel_aujourdhui': 'Demande CarTel • Aujourd\'hui',
      'demande_cartel_10_oct': 'Demande CarTel • 10 Oct',
      'demande_cartel_08_oct': 'Demande CarTel • 08 Oct',
      'identification_vehicules': 'Identification des véhicules',
      'attente_assignation_agent': 'Attente d\'assignation agent',
      'livre': 'Livré',
      'vehicule_trouve': 'Véhicule Trouvé',
      'statut': 'Statut',
      'resultat': 'Résultat',
      'paiement': 'Paiement',
      'confirme': 'Confirmé',
      'top_match': 'Top Match',
      'hors_budget': 'Hors Budget',
      'chat_agent': 'Discussion avec l\'agent...',
      'match_accepte': 'Offre acceptée',
      'match_refuse': 'Offre refusée',
      'accepter_offre': 'Accepter cette offre ?',
      'confirm_accept_match_desc': 'Voulez-vous accepter cette offre ? Cette action est irréversible.',
      'refuser_offre': 'Refuser cette offre ?',
      'confirm_decline_match_desc': 'Voulez-vous refuser cette offre ?',
      'match_accepte_success': 'Offre acceptée avec succès',
      'match_refuse_success': 'Offre refusée avec succès',
      'traitement': 'Traitement...',
      'refuser': 'Refuser',
      'notif_match_title': 'Nouveau Match Véhicule',
      'notif_match_desc': 'L\'agent Marcus a trouvé un Porsche Cayenne Coupe S 2023 correspondant à vos critères.',
      'notif_assign_title': 'Agent Assigné',
      'notif_assign_desc': 'Le spécialiste Marcus K. a été assigné à votre recherche #CT-8921.',
      'notif_pay_title': 'Paiement Confirmé',
      'notif_pay_desc': 'Frais de service de 30 000 FCFA reçus pour la demande #CT-8921.',
      'starting_from': 'À Partir De',
      'just_now': 'À l\'instant',
      '2_hours_ago': 'Il y a 2 heures',
      'yesterday': 'Hier',
      'hero_title': 'Laissez-nous trouver\nvotre véhicule idéal.',
      'hero_subtitle': 'Recherche personnalisée, achat et livraison sécurisée.',
      'modele_2024': 'Modèle 2024',
      'testimonial_text': '"CarTel a sourcé mon G-Wagon directement de Dubaï. Les mises à jour de transit étaient précises et la livraison au port sans encombre."',
      'preferences': 'Préférences',
      'support_legal': 'Support et Légal',
      'premium_member': 'Membre Premium',
      'currency': 'Devise',
      'help_center': 'Centre d\'aide',
      'privacy_policy': 'Politique de confidentialité',
      'contact': 'Contact',
      'help_center_title': 'À votre écoute',
      'help_center_subtitle': 'Notre équipe d\'experts est là pour vous accompagner dans vos recherches automobiles.',
      'write_us': 'Nous écrire',
      'follow_us': 'Suivez-nous',
      'customer_service_hours': 'Service client disponible 7j/7\nDe 08h00 à 20h00 (GMT+1)',
      'log_out': 'Se déconnecter',
      'consulter_rapport': 'Consulter le rapport final',
      'country_of_residence': 'Pays de résidence',
      'pricing_from': 'À Partir De',
      'model_label': 'Modèle',
      'start_sourcing': 'Démarrer le Sourcing',
      'kilométrage': 'Kilométrage',
      'énergie': 'Énergie',
      'fuel_petrol': 'Essence',
      'fuel_diesel': 'Diesel',
      'fuel_electric': 'Électrique',
      'fuel_hybrid': 'Hybride',
      'trans_auto': 'Auto',
      'trans_manual': 'Manuelle',
      'payment_success': 'Paiement Réussi',
      'receipt_email_sent': 'Un reçu a été envoyé à votre adresse e-mail.',
      'payment_failed': 'Paiement Refusé',
      'try_again': 'Réessayer',
      'change_method': 'Changer de Méthode',
      'track_my_request': 'Suivre ma Demande',
      'back_to_home': 'Retour à l\'Accueil',
      'transaction_id': 'ID de Transaction',
      'service_fee': 'Frais de Service',
      'amount': 'Montant',
      'error_code': 'Code d\'Erreur',
      'active_search': 'Recherche Active',
      'transaction_failed': 'Transaction Échouée',
      'payment_activated_desc': 'Votre demande de recherche a été activée. Un agent dédié commencera à sourcer votre Porsche Urus immédiatement.',
      'payment_failed_desc': "Nous n'avons pas pu traiter votre paiement. Veuillez vérifier votre solde ou essayer une autre méthode.",
      'create_request': 'Créer une demande',
      'vehicle_identity': 'Identité du véhicule',
      'brand': 'Constructeur',
      'investment_range': 'Gamme d\'investissement',
      'minimum': 'Minimum',
      'maximum': 'Maximum',
      'production_year': 'Année de production',
      'condition': 'État',
      'new': 'Neuf',
      'used': 'Occasion',
      'aesthetics': 'Esthétique',
      'exterior_shade': 'Couleur extérieure',
      'interior_shade': 'Couleur intérieure',
      'special_requirements': 'Besoins spéciaux',
      'describe_requirements': 'Décrivez vos préférences spécifiques...',
      'submit_request': 'Soumettre la Demande',
      'optional': 'Optionnel',
      'all': 'Toutes',
      'suv': 'SUV',
      'sports': 'Sportive',
      'sedan': 'Berline',
      'coupe': 'Coupé',
      'convertible': 'Cabriolet',
      'hatchback': 'Compacte',
      'pickup': 'Pick-up',
      'wagon': 'Break',
      'van': 'Van',
      'awd': 'Traction Intégrale',
      'fwd': 'Traction Avant',
      'rwd': 'Propulsion',
      '4wd': '4x4',
      '4x4': '4x4',
      'mileage_range': 'Gamme de Kilométrage',
      'kilometers': 'km',
      'accept_terms_prefix': 'J\'accepte les',
      'terms_link': 'Conditions d\'Utilisation',
      'and_label': 'et la',
      'privacy_link': 'Politique de Confidentialité',
      'create_account_btn': 'Créer mon compte',
      'login_title': 'Bon retour',
      'login_subtitle': 'Connectez-vous pour continuer votre recherche',
      'no_requests_found': 'Aucune demande trouvée',
      'no_trending_cars': 'Aucune voiture tendance disponible',
      'expertise_agent': 'Expertise de l\'Agent',
      'default_expertise_note': "Ce véhicule est dans un état exceptionnel, première main, carnet d'entretien Mercedes à jour. Le pack AMG Line et le toit ouvrant panoramique en font une configuration rare sur le marché.",
      'verdict_agent': 'Verdict de l\'Agent',
      'specialiste_luxe': 'Spécialiste Luxe & Performance',
      'fiche_technique': 'Fiche Technique',
      'marque': 'Marque',
      'modele': 'Modèle',
      'annee': 'Année',
      'kilometrage': 'Kilométrage',
      'moteur': 'Moteur',
      'boite': 'Boîte',
      'peinture_ext': 'Extérieur',
      'cuir_int': 'Intérieur',
      'prix_exportation': 'Prix Exportation',
      'price_label': 'Prix',
      'hot_deals': 'Offres Exceptionnelles',
      'affordable': 'Abordable',
      'best_price': 'Meilleur Prix',
      'interested': 'Intéressé(s)',
      'delete_request_title': 'Supprimer la demande ?',
      'delete_request_msg': 'Cette action est irréversible. Voulez-vous continuer ?',
      'cancel': 'Annuler',
      'delete': 'Supprimer',
      'request_deleted': 'Demande supprimée avec succès',
      'request_delete_error': 'Erreur: Impossible de supprimer la demande. RLS ou ID incorrect.',
      'step': 'Étape',
      'of': 'Sur',
      'confirmation': 'Confirmation',
      'search_summary': 'Résumé de la Recherche',
      'search_request': 'Demande de Recherche',
      'currency_label': 'Devise',
      'payment_method': 'Méthode de Paiement',
      'secure_transaction_msg': 'Transactions sécurisées et chiffrées de bout en bout.',
      'pay_amount': 'Payer',
      'payment_failed_prefix': 'Paiement échoué',
      'payment_cancelled_prefix': 'Paiement annulé ou échoué',
      'unexpected_error': 'Une erreur est survenue',
      'onboarding': 'Intégration',
      'identity': 'Identité',
      'condition_label': 'État',
      'investment': 'Investissement',
      'specs': 'Spécifications',
      'custom': 'Personnalisé',
      'e_g_urus': 'ex. Urus',
      'no_deals_available': 'Aucune offre disponible',
      'model_suffix': 'Modèle',
      'searching': 'En Recherche',
      'cancel_request': 'Annuler la Demande',
      'cartel_search': 'Recherche Cartel',
      'file_status': 'Du Dossier',
      'no_matches_found': 'Aucun match trouvé',
      'paiement_valide': 'Paiement Validé',
      'agent_assigne': 'Agent Assigné',
      'attente_assignation': 'En attente d\'assignation...',
      'recherche_active': 'Recherche Active',
      'identification_en_cours': 'Identification des véhicules en cours...',
      'en_attente': 'En attente...',
      'rapport_final': 'Rapport Final',
      'attente_resultats': 'En attente de résultats',
      'expert_luxe_actif': 'Expert Luxe • Actif',
      'note_logistique': 'Note Logistique',
      'shipping_fee_prefix': "Frais d'expédition internationale à partir de",
      'no_notifications': 'Aucune notification disponible',
      'clear_all': 'Tout effacer',
      'clear_all_notifications_title': 'Effacer toutes les notifications ?',
      'clear_all_notifications_desc': 'Cette action est irréversible.',
      'swipe_to_decide': 'Glissez à Droite pour Accepter • Gauche pour Refuser',
      'unassigned': 'Non assigné',
      'match': 'Match',
      'found': 'Trouvé',
      'assignment': 'Assignation',
      'agent_assigned': 'Assignation',
      'payment': 'Paiement',
      'payment_confirmed': 'Paiement',
      'current_selection': 'Sélection actuelle',
      'customs_fees': 'Frais de Douane',
      'customs_fees_msg': 'Les frais de dédouanement ne sont pas inclus dans les prix affichés.',
      'online': 'En ligne',
      'luxury_expert': 'Expert Luxe',
      'typing': 'écrit...',
      'write_message': 'Écrivez votre message...',
      'today': "Aujourd'hui",
      'chat_with_agent': 'Discussion',
      'body_style': 'Silhouette',
      'drive_train': 'Transmission',
      'features': 'Équipements',
      'equipements_inclus': 'Équipements inclus',
      'awaiting_payment': 'En attente de paiement',
      'payment_pending_short': 'En attente',
      'payment_declined_short': 'Refusé',
      'match_accepted_step': 'Match accepté · Paiement en attente',
      'match_paid_step': 'Paiement final confirmé',
      'in_transit': 'En Transit',
      'in_transit_step': 'Véhicule en transit',
      'delivered_step': 'Véhicule livré · En attente de confirmation',
      'support_center': 'Support CarTel',
      'support_chat_empty': 'Envoyez un message pour discuter avec notre équipe de support.',
      'privacy_last_updated': 'DERNIÈRE MISE À JOUR : OCT 2023',
      'privacy_title': 'Politique de\nConfidentialité',
      'privacy_subtitle': 'Chez CarTel, nous accordons une importance primordiale à la protection de vos données personnelles. Cette politique détaille comment nous collectons et traitons vos informations.',
      'privacy_data_collection': 'Collecte des Données',
      'privacy_data_collection_label': 'Informations fournies par l\'utilisateur',
      'privacy_data_collection_body': 'Nous collectons votre nom, e-mail, numéro de téléphone et les détails relatifs à vos recherches de véhicules lors de la création de demandes.',
      'privacy_data_usage': 'Utilisation des Données',
      'privacy_data_usage_label': 'Finalités du traitement',
      'privacy_data_usage_body': 'Vos données servent exclusivement à faciliter la recherche de votre véhicule par nos agents et à assurer le suivi de vos transactions.',
      'privacy_security': 'Sécurité & Partage',
      'privacy_security_body': 'CarTel ne vend jamais vos données à des tiers. Nous utilisons des protocoles de chiffrement avancés pour garantir l\'intégrité de vos informations.',
      'privacy_questions': 'Des questions ?',
      'privacy_contact_subtitle': 'Contactez notre délégué à la protection des données via support@cartel.app',
      'privacy_contact_btn': 'Contacter le Support',
      'privacy_copyright': '© 2023 CarTel Automobile. Tous droits réservés.',
    },
  };

  String translate(String key) {
    return _translations[_currentLanguage]?[key] ?? key;
  }
}
